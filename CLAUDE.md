...
- Los **enums** del esquema (`place_origin`, `social_network`, etc.) se mapean como enums nativos de
  PostgreSQL: `MapEnum<T>` en el data source + `HasPostgresEnum<T>` en el `DbContext`. Los nombres de
...
- **Swagger/OpenAPI** habilitado. **CORS** configurado (habrá un frontend Angular PWA).
